# -*- coding: utf-8 -*-
"""bert_large_shared_v1.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/10Q_9fq0kGpBd64J-tDeJZsgR9iwksL6r
"""
...
config = BertConfig.from_pretrained(
        "bert-large-uncased",
        num_labels=2)
vibert_large_shared = BertForSequenceClassification.from_pretrained(
        "bert-large-uncased",
        config=config
)

for name, param in vibert_large_shared.named_parameters():
...
optimizer_grouped_parameters = [
        {
            "params": [p for n, p in vibert_large_shared.named_parameters() if not any(nd in n for nd in no_decay)],
            "weight_decay": 0.0,
        },
        {"params": [p for n, p in vibert_large_shared.named_parameters() if any(nd in n for nd in no_decay)], "weight_decay": 0.0},
]
...
n_epochs = 10
device="cuda"
#vibert_large_shared.train()
vibert_large_shared.to(device)
vibert_large_shared.zero_grad()
set_seed(2022)
for epoch in range(n_epochs):
  ...
  for labels,inp1,inp2,st in tqdm(train_dataloader):
    ...
    out = vibert_large_shared(batch["input_ids"],token_type_ids=batch["token_type_ids"], 
    ...
    if st[0]=="mrpc":
      ...
      c2_train+=1
    torch.nn.utils.clip_grad_norm_(vibert_large_shared.parameters(), 1.0)
  ...
  torch.save(vibert_large_shared.state_dict(), "vibert_large_shared_70")
